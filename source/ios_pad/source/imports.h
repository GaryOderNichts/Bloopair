...
extern int (*const IOS_GetCurrentThreadID)(void);
extern int (*const IOS_StartThread)(int threadid);
extern int (*const IOS_SuspendThread)(int threadid);
...
extern int (*const IOS_SendMessage)(int queueid, uint32_t message, uint32_t flags);
...
extern void (*const bta_hh_snd_write_dev)(uint8_t dev_handle, uint8_t t_type, uint8_t param, uint16_t data, uint8_t rpt_id, BT_HDR *p_data);
...
extern uint8_t (*const BTM_WriteStoredLinkKey)(uint8_t num_keys, uint8_t *bd_addr, uint8_t *link_key, void *p_cb);
